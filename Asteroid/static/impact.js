(function(){
  ...
  // Bar chart of effects using Chart.js (if available)
    ...
    const chart = new Chart(chartCanvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          ...
          title: { display: true, text: `Energy: ${metrics.energy_megatons_tnt} Mt | Severity: ${metrics.severity}` },
          tooltip: {
            displayColors: false,
            padding: 6,
            cornerRadius: 6,
            ...
            callbacks: {
              title: (items) => items?.[0]?.label?.join ? items[0].label.join(' ') : items?.[0]?.label || '',
        ...
        scales: {
          y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,0.08)' }, title: { display: true, text: 'km' }, ticks: { font: { size: 10.5 } } },
          x: { ticks: { font: { weight: '600', size: 10.5 } }, title: { display: true, text: 'Effect' } }
        }
      }
    });
  }
